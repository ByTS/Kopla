...
app.controller("yhteydet", function($scope, $http, orderByFilter) {
	$http.get('https://rata.digitraffic.fi/api/v1/live-trains?station=TKL&departed_trains=0&departing_trains=10&arriving_trains=0&arrived_trains=0').
    success(function(data, status, headers, config) {
      $scope.tkl = data;
      $scope.sortedtkl = orderByFilter($scope.tkl, '+scheduledTime');
      console.log($scope.sortedtkl);
      });
	$http.get('https://rata.digitraffic.fi/api/v1/live-trains?station=HPL&departed_trains=0&departing_trains=10&arriving_trains=0&arrived_trains=0').
    success(function(data, status, headers, config) {
      $scope.hpl = data;
...
<span ng-show="y.stationShortCode=='TKL'" ng-controller="yhteydet" class="ng-cloak">
	<span ng-repeat="x in tkl | orderBy:'scheduledTime'" ng-init="x.scheduledTime = (x.timeTableRows | filter:{stationShortCode:'TKL', 'type':'DEPARTURE'})[0].scheduledTime" ng-hide="x.cancelled || x.trainCategory === 'Long-distance'">
		<br>{{x.commuterLineID}} {{x.trainNumber}} <span ng-repeat="y in x.timeTableRows | limitTo:-1"> -> {{y.stationShortCode}}</span> {{x.scheduledTime | date:'HH:mm'}} R.<span ng-repeat="y in x.timeTableRows | filter:{'stationShortCode':'TKL', 'type':'DEPARTURE'}: true | limitTo:1">{{y.commercialTrack}}</span>
	</span>
</span>
